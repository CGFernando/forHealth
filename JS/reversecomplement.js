function doReverseComplement(){
    ...
    res.innerHTML = `${seq.split('').reverse().map(elem => DNADNAcomplement[elem]).join('')}`
    } else if (typeSeqChoose === 'DNARNA' && /^[ACTG]+$/.test(seq)) {
    res.innerHTML = `${seq.split('').reverse().map(elem => DNARNAcomplement[elem]).join('')}`
    } else if (typeSeqChoose === 'RNADNA' && /^[ACUG]+$/.test(seq)) {
    res.innerHTML = `${seq.split('').reverse().map(elem => RNADNAcomplement[elem]).join('')}`
    } else {
